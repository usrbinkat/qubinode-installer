...
function ensure_supported_ansible_version () {
    { # try
       ANSIBLE_VERSION=$(awk '/ansible_version/ {print $2}' "${vars_file}")
        ...
        printf "%s\n" "   ${red}Skipping ANSIBLE_VERSION setting.${end}"
    }

    { # try
       ANSIBLE_RELEASE=$(awk '/ansible_release/ {print $2}' "${vars_file}}")
    } || { # catch
        printf "%s\n" "   ${red}Skipping ANSIBLE_RELEASE setting.${end}"
    }
    ...
    { # try
       ANSIBLE_RPM=$(awk '/ansible_rpm/ {print $2}' "${vars_file}")
        ...
        printf "%s\n" "   ${red}Skipping ANSIBLE_RPM setting.${end}"
    }
...
function qubinode_setup_ansible () {
    ...
    then
        ...
        then
            ...
            sudo yum install -y -q -e 0 python3 python3-pip python3-dns python3-devel gcc> /dev/null 2>&1
    ...
    if [[ ${RHEL_VERSION} == "RHEL7" ]]; then
        ...
        then
            ...
            sudo yum install -y -q -e 0 python python3-pip python2-pip python-dns python-devel gcc > /dev/null 2>&1
    ...
    then
        ...
        { # try
         ...
         CURRENT_REPO=$(sudo subscription-manager repos --list-enabled| awk '/ID:/ {print $3}'|grep ansible)
            ...
            printf "%s\n" "   ${red}ANSIBLE Current REPO NOT FOUND${end}"
        }
        # check to make sure the support ansible repo is enabled
        if [ "A${CURRENT_REPO}" != "A${ANSIBLE_REPO}" ]
        then
            if [ ! -z ${CURRENT_REPO} ]; then sudo subscription-manager repos --disable="${CURRENT_REPO}"; fi 
            sudo subscription-manager repos --enable="${ANSIBLE_REPO}"
        fi

        printf "%s\n" "   ${grn}Installing ansible${end}"
        if [[ ${RHEL_VERSION} == "RHEL8" ]]; then
            sudo dnf clean all > /dev/null 2>&1
    ...
    then
        ...
        then
            #printf "%s\n" " Encrypting ${vaultfile}"check_rhsm_status
            ansible-vault encrypt "${vaultfile}" > /dev/null 2>&1
        fi

        # Ensure roles are downloaded
        if [ "${qubinode_maintenance_opt:-skip}" == "ansible" ]
        then
        ...
        then
            ...
            CURRENT_DIR=$(pwd)
            cd "${project_dir}/playbooks/modules/" || exit
...
function decrypt_ansible_vault () {
    ...
    { # try
        grep -Fxq VAULT "${vaultfile}";
        if [ "A$?" != "A0" ];
        then
            cd "${project_dir}/" 
            if [ -f /usr/bin/ansible-vault ]; then  ansible-vault decrypt "${vaultfile}"; fi 
            ansible_encrypt=yes
        fi
    } || { # catch
        printf "%s\n" "   ${red}Skipping vault decryption.${end}"
    }
}
...
function encrypt_ansible_vault () {
    ...
    { # try
        ...
        then
            cd "${project_dir}/" 
            if [ -f /usr/bin/ansible-vault ]; then  ansible-vault encrypt "${vaultfile}"; fi 
        fi
    } || { # catch
        printf "%s\n" "   ${red}Skipping vault encryption.${end}"
    }
